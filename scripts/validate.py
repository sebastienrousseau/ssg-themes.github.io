...
def check_registration(root: Path) -> list[str]:
    ...
    # A query string on an asset href defeats the generator's fingerprint
    # rewrite: `styles.css?v=0.0.4` is not rewritten to
    # `styles.<hash>.css`, and the unfingerprinted name is never emitted, so
    # the reference 404s silently. Voxt shipped exactly this on all three of
    # its assets -- no stylesheet and no JavaScript at all -- and it was
    # invisible because a missing stylesheet still renders a readable page.
    for theme_dir in sorted(d for d in (root / "themes").iterdir() if d.is_dir()):
        layouts = theme_dir / "_layouts"
        if not layouts.is_dir():
            continue
        for html in sorted(layouts.glob("*.html")):
            text = html.read_text(encoding="utf-8")
            for ref in re.findall(r'(?:src|href)="([^"]*\.(?:css|js))\?[^"]*"', text):
                errors.append(
                    f"{theme_dir.name}/_layouts/{html.name}: "
                    f'"{ref}" carries a query string; that blocks asset '
                    "fingerprinting and the reference will 404"
                )

    # The landing page is generated by ssg from showcase/content/index.md
    # using the Apex theme, so the source of truth for these claims is that
    # content file -- not a hand-authored public_index.html, which was
    # removed. That file was a second, ungenerated copy of the same page: it
    # drifted to "Eight themes" with nine on disk, and a third-party CSS
    # framework was once pasted over its styles without any gate noticing,
    # precisely because nothing about it was generated.
    index = root / "showcase" / "content" / "index.md"
    if index.is_file():
        ...
        for name in on_disk:
            ...
            if f'href="{name}/"' not in html:
                errors.append(
                    f"showcase/content/index.md has no card linking to {name}/"
                )
    else:
        errors.append("showcase/content/index.md is missing")
            ...
            for n, word in words.items():
                ...
                for phrase in (f"{word} themes", f"{word} gated themes"):
                    if phrase in lowered:
                        errors.append(
                            f'showcase/content/index.md says "{phrase}" but themes/ '
                            f"holds {len(on_disk)}"
                        )
            if f"{expected} themes" not in lowered:
                errors.append(
                    f'showcase/content/index.md never says "{expected} themes"; '
        ...
        for name in on_disk:
            ...
            if tj.is_file():
                try:
                    ...
                    cat = None
                if cat:
                    # Matched on the pair, not on exact whitespace: the
                    ...
                    found = False
                    pos = html.find(eyebrow)
                    while pos != -1:
                        if heading in html[pos:pos + 400]:
                            found = True
                            break
    ...
    # The facts live in the showcase *layout*, not the content file: they are
    # page furniture rendered around the content, and scripts/facts.py fills
    # them in the built page.
